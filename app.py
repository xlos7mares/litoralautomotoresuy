...
st.markdown("""
    ...
    .btn-ws { display: block; width: 100%; text-align: center; color: white !important; padding: 15px; margin: 10px 0; border-radius: 8px; font-weight: bold; text-decoration: none; font-size: 1.1em; }
    .btn-info { background-color: #007bff; }
    .btn-interesa { background-color: #dc3545; }
    </style>
    """, unsafe_allow_html=True)

# --- LÓGICA DE NAVEGACIÓN (FOTO EN GRANDE / ZOOM) ---
...
def mostrar_auto(titulo, precio, caracteristicas, lista_fotos, ws_msg):
    ...
    if lista_fotos:
        ...
        if st.button(f"🔍 Ver en Grande", key=f"main_{lista_fotos[0]}"):
    ...
    st.markdown(f'<a href="{ws_url}" target="_blank" class="btn-ws btn-info">💬 CONSULTAR INFO</a>', unsafe_allow_html=True)
    st.markdown(f'<a href="{ws_url}" target="_blank" class="btn-ws btn-interesa">🔥 ME INTERESA</a>', unsafe_allow_html=True)
...
with col1:
    mostrar_auto("Suzuki Vitara GL (2016)", "USD 14.800.-", 
                 ["150.000 km | Servicio al día", "4 cubiertas nuevas", "Muy sana, sin detalles"], 
...
with col3:
    mostrar_auto("HYUNDAI ACCENT 1.5 (1995)", "¡ECONÓMICO!", 
                 ["1.5 Nafta", "Títulos al día", "Listo para transferir"], 
